...
0. **⭐ Validación por movimiento (idea de Carlos, la killer feature)** — *Grande* · Carlos activa "grabar recorrido", empuja su silla, y **la app valida como accesible todo el camino que hizo** (si una silla pasó, es transitable = prueba viva). Si se detiene un rato, un aviso suave lo invita a reportar (¿algo te lo impidió?). Un recorrido grabado ES un tramo verde (el modelo `tramo` ya lo soporta). **Constraint clave:** un navegador NO rastrea GPS en segundo plano / pantalla apagada; la versión pasiva de verdad (teléfono en el bolsillo/soporte) **necesita PWA** — que además le conviene a Carlos por su movilidad de manos (no sostener el teléfono con pantalla prendida). → hacer la **PWA** primero desbloquea esto bien. Privacidad: opt-in por viaje, visible mientras graba, su rastro es suyo; el tramo público conviene generalizarlo (no exponer su puerta/rutina).

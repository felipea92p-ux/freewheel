...
00. ~~Correr `supabase-tramos.sql`~~ ✅ Hecho (2026-07-18, verificado contra la base viva).
...
0. **⭐ Validación por movimiento (idea de Carlos)** — v1 **HECHA** (primer plano). Falta: **background/pantalla apagada = app nativa** (iPhone no lo permite ni en PWA); y **generalizar el rastro público** (no exponer puerta/horario de Carlos). Privacidad v2.
...
- **[Carlos] Marcar tramo** — 1ª petición del usuario real: marcar una **calle/cuadra entera** accesible, no solo un punto. Botón "📏 Marcar tramo" → toca inicio y final → línea (recta v1) guardada como reporte con geometría, pintada del color de su gravedad (**verde/ámbar/rojo aquí SÍ es honesto**: lo afirma una persona). Columna nueva `tramo` jsonb (punto O tramo, misma tabla/RLS); payload manda `tramo` solo cuando lo es (no rompe reportes normales si falta la columna). Verificado: syntax, flujo DOM (botón→modo→hint→cancelar), `rebuildTramos` con datos de prueba, sin errores de consola, layout. **Falta correr el SQL** (pendiente #00) y el round-trip visual en celular (trazar→guardar→línea de color). Ahora **multipunto** (toca esquina por esquina las calles que doblan, "Listo" para terminar; tramo nuevo pre-elegido en verde/accesible, editable).
- **[ruta] Alternativas con estilo neutro ("A")** — la ruta pide hasta 3 caminos (ORS `alternative_routes`); la activa índigo, las otras **gris punteado neutro a propósito** (todas son accesibles; verde/ámbar/rojo mentiría). Tocarla la vuelve principal y actualiza km/min. Verificado: ORS real en Trujillo devolvió 3 rutas (2.83/3.29/3.63 km) con la petición exacta de la app; render/switch/mensaje probados con esos datos; sintaxis OK. Falta el toque visual en celular. El semáforo real por barreras = Fase 3.
- **[fix] Mensajes de guardado honestos** — ya no culpa a la conexión cuando el rechazo es de la base (columna faltante → "avisa a Cayla"; red → "revisa tu conexión"; otro → "el servidor lo rechazó"). Clasificación probada con los 4 tipos de error.
- **[infra] Columna `tramo` creada en producción** (Cayla corrió el SQL; verificada contra la base viva — reportes viejos intactos).
- **[Carlos] Grabar recorrido (v1, primer plano)** — botón "⏺ Grabar recorrido": `watchPosition` dibuja el rastro en vivo, "Detener" → tramo (hoja verde, editable/descartable). Filtra ruido (ignora <8m o accuracy>60m), mide distancia, Wake Lock para no apagar pantalla, aviso suave si lleva rato parado. Verificado: sintaxis, DOM (inicia/estado/detener), limpieza ante error de GPS, sin errores de consola, y matemática de distancia + filtrado con datos de prueba. Round-trip real (moverse y ver la línea) = celular de Carlos. **Fondo/pantalla apagada = app nativa** (ver Ideas #0).
